...
void red_panic(const char* file, size_t line, const char* function, const char* format, ...)
{
    ...
    sprintf(buffer2, "Panic at %s:%zu: In function %s, %s\n", file, line, function, buffer);
#ifdef RED_OS_WINDOWS
...
void os_get_callstack(const char* file, size_t line, const char* function)
{
    ...
    char** strings = NULL;
    s32 count = backtrace((void*)buffer, array_length(buffer));
    if (count > 0)
    {
        strings = backtrace_symbols(buffer, count);
        if (strings != NULL)
        {
            // print("Panic happened at %s:%zu:%s\n\n", file, line, function);
            print("Callstack:\n");
            // We skip first function because it's the callstack function
        ...
        {
            print("Couldn't backtrace symbols\n");
        }
    }
    else
    {
        print("Backtrace information couldn't be obtained\n");
    }
#else
#error
#endif
